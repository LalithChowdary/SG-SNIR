...
\textbf{Comparison against DINO [2]:} DINO [2] is a node immunization strategy that greedily removes nodes to minimize the spectral radius of the KSCC. However, removing whole nodes is often practically impossible in real-world quarantines. To create a fair comparison against SG-SNIR's targeted edge-blocking approach, we use an \emph{equivalent contact budget}. We ran the original DINO algorithm on p2p-Gnutella to progressively select $k \in \{1, 2, 3, 4, 5\}$ nodes to remove. At each step, we measured the equivalent contact budget (the cumulative number of edges removed by isolating those nodes). For example, isolating $k=1$ node removes 97 edges, and $k=5$ nodes removes 459 edges. Evaluating these DINO interventions yielded a final expected influence that plateaued around $H_{\text{final}} = 19.80$.

We then provided SG-SNIR with the exact same intervention budgets. Across all tested budgets, SG-SNIR achieved a significantly lower final influence of $H_{\text{final}} = 15.63$. Strikingly, SG-SNIR completely halts the epidemic using only 15 targeted edge removals, achieving a \textbf{21.1\% reduction in epidemic spread} compared to DINO's maximum budget of 459 edges. This conclusively validates our core argument: using structural metrics alone (like DINO) wastes intervention budget on non-transmitting contacts. By combining DINO's spectral guidance with SNIR simulation awareness, SG-SNIR isolates the specific contacts responsible for the epidemic, achieving vastly superior containment for a fraction of the cost.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\linewidth]{../plots/diagrams/budget_comparison_line.pdf}
\caption{Equivalent Contact Budget comparison on p2p-Gnutella across varying DINO node-removal budgets ($k=1$ to $5$ nodes). SG-SNIR completely isolates the epidemic using just 15 targeted edge removals, whereas DINO wastes hundreds of edge removals without effectively halting the spread.}
