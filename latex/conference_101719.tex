...
Table~\ref{tab:effectiveness} reports the final cumulative influence $H$ after $k=20$ edge removals on three datasets across four experimental configurations (2 $\eta$ settings $\times$ 2 weight modes). SG-SNIR consistently outperforms DegreeProduct and Random baselines across all 12 configurations.
...
\hline
HIV & \multicolumn{2}{c}{Equal (by construction)} & 0.000\% \\
p2p-Gnutella & 20.797 & 20.775 & $+$0.106\% \\
Wiki-Vote & 22.149 & 21.660 & $+$2.256\%\textsuperscript{$\dagger$} \\
\hline
\multicolumn{4}{l}{\textsuperscript{$\dagger$}3 trials; gap driven by one outlier seed (see text).}
\end{tabular}
\end{table}
...
On \textbf{soc-Epinions1} ($k=10$, 1 trial), SG-SNIR uses 286 evaluations vs.\ 445 for exhaustive search, a 1.56$\times$ speedup. The quality gap is $+$14.1\%, reflecting both the reduced budget ($k=10$) and the large candidate pool on a 508K-edge graph. This single-trial result is indicative rather than definitive but demonstrates that the filter remains active on large networks.
...
We subjected both algorithms to massive, globally distributed epidemics by initializing the SNIR simulation with 300 and 600 random seed nodes in the p2p-Gnutella network. We ran DINO to incrementally select varying budgets of top hub nodes to quarantine. For 300 seeds, quarantining up to 15 central nodes severed exactly 1,256 edges. For 600 seeds, quarantining up to 20 nodes severed 1,607 edges. We then provided SG-SNIR with the exact same maximum edge-disruption budgets and compared the resulting expected final influence $H_{\text{final}}$.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\linewidth]{../plots/diagrams/budget_comparison_300_seeds.pdf} \\
\vspace{0.2cm}
\includegraphics[width=0.9\linewidth]{../plots/diagrams/budget_comparison_600_seeds.pdf}
...
\noindent [1] Dai, X.~et al., ``SNIR: A Probabilistic Model for Epidemic Propagation,'' \textit{IEEE TPDS}, 2022.

\noindent [2] He, Z.~et al., ``DINO: Directed Network Immunization,'' \textit{Proc.\ ACM KDD}, 2023.
...
\noindent [5] Tarjan, R.\ E., ``Depth-first search and linear graph algorithms,'' \textit{SIAM J.\ Comput.}, 1(2):146--160, 1972.
