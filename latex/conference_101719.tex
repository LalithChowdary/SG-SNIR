...
\textbf{Comparison against DINO [2]:} DINO [2] is a node immunization strategy that greedily removes nodes to minimize the spectral radius of the KSCC. However, removing whole nodes is often practically impossible in real-world quarantines. To create a fair comparison against SG-SNIR's targeted edge-blocking approach, we use an \emph{equivalent contact budget}. We ran the original DINO algorithm on p2p-Gnutella to select $k=5$ nodes to remove. Removing these 5 nodes resulted in the deletion of exactly 459 edges (contacts). Evaluating this DINO intervention using the SNIR simulator yielded a final expected influence of $H_{\text{final}} = 19.80$.

We then provided SG-SNIR with the exact same intervention budget of 459 edges. SG-SNIR achieved a significantly lower final influence of $H_{\text{final}} = 15.63$, representing a \textbf{21.1\% reduction in epidemic spread} (a quality gap improvement of 4.17) for the exact same number of blocked contacts. This conclusively validates our core argument: using structural metrics alone (like DINO) wastes intervention budget on non-transmitting contacts. By combining DINO's spectral guidance with SNIR simulation awareness, SG-SNIR isolates the specific contacts responsible for the epidemic, achieving vastly superior containment for the same intervention cost.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\linewidth]{../plots/diagrams/budget_comparison_bar.pdf}
\caption{Equivalent Contact Budget comparison on p2p-Gnutella. SG-SNIR reduces epidemic influence by 21.1\% compared to DINO when given the exact same budget of 459 removed contacts.}
\label{fig:budget_bar}
\end{figure}
